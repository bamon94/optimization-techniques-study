...
def plot_contourf(cost_f, figsize=[10, 10], _show=True,title=""):
    ...
    for i in range(len(cost_f.x_optimum)):
        plt.text(cost_f.x_optimum[i], cost_f.y_optimum[i],"x", color="b", size=20)
    if _show:
        plt.show()
    return fig, ax


def plot_contourWithTrajectory(cost_f,trajectories_dict, figsize=[10, 10],_show=True,title=""):
    fig = plt.figure(figsize=figsize)
    ax=fig.gca()
    x = np.arange(cost_f.xmin, cost_f.xmax, 0.1)
    y = np.arange(cost_f.ymin, cost_f.ymax, 0.1)
    ...
    for name, (x, y, c) in trajectories_dict.items():
        ax.plot(x, y, color=c, zorder=1, linewidth=2)
...
def plot_cost_function_3d(cost_f,title="",grain=0.01, figsize=[10,6]):
    #plt.title("3D plot of log Beale")
